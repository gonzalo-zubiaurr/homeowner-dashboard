...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[upload-csv] Headers:', headers.length, 'Rows:', lines.length - 1)
    ...
    if (homes.length) {
      ...
      if (error) console.error('[upload-csv] Homes error:', error)
    }
    ...
    const leaseRows = rows.map(row => {
      ...
      return {
        ...
        homeowner_name: get(row, 'HomeownerName'), homeowner_id: get(row, 'HomeownerId') || null,
        homeowner_link: get(row, 'HomeownerLink') || null, intercom_link: get(row, 'IntercomLink') || null,
        escalation_slack_link: get(row, 'EscalationSlackLink') || null, lease_type: get(row, 'LeaseType'),
    ...
    for (let i = 0; i < leaseRows.length; i += 100) {
      ...
      if (error) { console.error('[upload-csv] Lease batch error:', i, error); throw error }
    }
    console.log('[upload-csv] Done:', leaseRows.length)
